// Application API wrapper for the application service
import { submitApplication, getUserApplications, updateApplicationStatus } from '../services/applicationService';
 ...
 * @param {string} token - The authentication token
 * @param {Object} params - Parameters containing application_id
 * @param {string} status - The new status
 * @returns {Promise<Object>} The updated application
 */
export async function updateStatus(token, { application_id }, status) {
  try {
    console.log(`API: Updating application ${application_id} status to ${status}`);
    
    // Call the service to update the application status
    const updatedApplication = await updateApplicationStatus(token, application_id, status);
