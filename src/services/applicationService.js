 ...
 */
async function uploadResumeFile(file, jobId, candidateId) {
  try {
 ...
 */
async function saveApplicationData(applicationData, serviceClient) {
  try {
    ...
    console.log("Saving application data:", filteredData);
 ...
 * @param {Object} applicationData - The application data including resume file
...
export async function submitApplication(token, applicationData) {
  ...
  try {
    ...
    console.log(`Found Supabase UUID ${supabaseUserId} for Clerk ID ${clerkId}`);
    ...
    const savedApplication = await saveApplicationData(dbApplicationData, serviceClient);
