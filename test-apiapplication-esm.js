...
console.log('Supabase URL in test-apiApplication-esm.js:', supabaseUrl);
console.log('Service Role Key in test-apiApplication-esm.js (first 10 chars):', serviceRoleKey ? serviceRoleKey.substring(0, 10) + '...' : 'undefined');
...
async function applyToJob(token, _, jobData) {
  ...
  try {
    ...
    const applicationData = {
      ...
      status: 'pending',
      name: jobData.name || 'Test Applicant',
      email: jobData.email || 'test@example.com',
      resume: resumeUrl, // Using resume field instead of resume_url
...
export { applyToJob, supabaseClient, supabaseUrl, createServiceRoleClient };
